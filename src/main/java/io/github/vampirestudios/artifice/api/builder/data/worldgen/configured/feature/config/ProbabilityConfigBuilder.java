...
public class ProbabilityConfigBuilder extends FeatureConfigBuilder {
	...
	public ProbabilityConfigBuilder probability(float probability) {
		if (probability > 1.0F)
			throw new IllegalArgumentException("probability can't be higher than 1.0F! Found " + probability);
		if (probability < 0.0F)
			throw new IllegalArgumentException("probability can't be smaller than 0.0F! Found " + probability);
