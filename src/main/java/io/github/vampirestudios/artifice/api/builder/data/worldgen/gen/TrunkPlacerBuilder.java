...
public class TrunkPlacerBuilder extends TypedJsonBuilder<JsonObject> {
	...
	public <P extends TrunkPlacerBuilder> P baseHeight(int base_height) {
		if (base_height > 32)
			throw new IllegalArgumentException("base_height can't be higher than 32! Found " + base_height);
		if (base_height < 0)
			throw new IllegalArgumentException("base_height can't be smaller than 0! Found " + base_height);
	...
	public <P extends TrunkPlacerBuilder> P heightRandA(int height_rand_a) {
		if (height_rand_a > 24)
			throw new IllegalArgumentException("height_rand_a can't be higher than 24! Found " + height_rand_a);
		if (height_rand_a < 0)
			throw new IllegalArgumentException("height_rand_a can't be smaller than 0! Found " + height_rand_a);
	...
	public <P extends TrunkPlacerBuilder> P heightRandB(int height_rand_b) {
		if (height_rand_b > 24)
			throw new IllegalArgumentException("height_rand_b can't be higher than 24! Found " + height_rand_b);
		if (height_rand_b < 0)
			throw new IllegalArgumentException("height_rand_b can't be smaller than 0! Found " + height_rand_b);
